...
///
/// Like for the editor you can either use the `onCreated(EditorApi)` callback
/// or a global key to get access to the state,
/// in this case the [PackagedHtmlEditorState]. With either the state or the
/// [HtmlEditorApi] you can access the edited text with
/// ```dart
/// String edited = await editorApi.getText();
/// ```
/// Alternatively call `editorApi.getFullHtml()` to retrieve a full HTML
/// document.
class PackagedHtmlEditor extends StatefulWidget {
  ...
  /// default `100` pixel.
  ///
  /// Define the [onCreated] `onCreated(EditorApi)` callback to get
  /// notified when the API is ready.
  ///
  /// Set [splitBlockquotes] to `false` in case block quotes should not be
  /// split when the user adds a newline in one - this defaults to `true`.
  const PackagedHtmlEditor({
    Key? key,
    this.initialContent = '',
  ...
  /// Set `excludeDocumentLevelControls` to `true` in case document level
  /// controls such as the page background color should be excluded.
